...
> 状态：已采用；M5.6.1 沉浸式战斗场景修复已完成，M5.7 熔炼规则仅完成设计，验收以 `docs/prototype-checklist.md` 为准。
...
- 突破使用固定材料和固定专属节点，不引入随机强化、武器词条重铸或第三套经验。装备重铸仍只改变装备附加词条。
- 持久化品阶时再将存档升为 v5，新增 `weaponTiers` 并将 v4 武器迁移为一阶；战斗只接收投影后的 `BattleModifiers`，不直接读取玩家存档。
