...
> 状态：已采用；M5.6.1 沉浸式战斗场景修复和 M5.7 器物熔炼/全图鉴美术已完成，验收以 `docs/prototype-checklist.md` 为准。
...
- 角色页按页签组织配装、妖灵、法宝丹药和术法；配装页只保留六个主槽，左侧为头冠/法衣/足履，右侧为武器/功法/佩饰。槽位使用 CSS Grid 正常流，不使用人物周围绝对定位轨道。
- `src/state/loadout.ts` 仍是换装、交换、预设识别和自动优先级的唯一规则入口。角色页不新增预留存档槽；未来系统以真实内容和独立页签进入。
- `src/game/combat-math.ts` 是有效护体、每段伤害、多段总伤害、普攻 DPS 和基础战力的唯一计算入口。`battle.ts`、角色页详情和卡牌预览必须复用它，不能在 JSX 复制公式。
- M5.6 保留现有伤害公式与 `saveVersion: 4`，只补齐可解释的伤害明细和基础战力展示。基础战力是“全队有效生元 + 10 秒无甲自动攻击伤害”，只用于比较，不参与战斗或奖励。
- 内容文案必须与 `effectParams` 和战斗分支一致；若行为与旧文案冲突，优先修正实际规则的状态恢复或减费目标，再更新详情文本并增加一致性测试。

## 16. M5.7 器物熔炼与全图鉴美术边界

- 精炼是唯一连续成长轴，武器与装备复用现有 `levels[id]`；Lv.1–10 为一阶凡器，Lv.10 突破后开放 Lv.11–20，突破不重置等级。
- v5 存档新增 `forgeTiers`，缺少记录视为一阶。v1–v4 迁移保留资源、收藏、等级、配装、主线、劫境、词条和志怪录。
- 突破使用固定材料和逐件固定节点，不引入随机强化、武器词条重铸或第三套经验。装备重铸仍只改变附加词条；免费重置退回一阶 Lv.1 并返还精炼与突破材料。
- `src/content/forging.ts` 是 15 个二阶节点与参数覆盖的唯一来源；详情、角色页、`battleContentFromSave` 和战斗规则都读取同一份数据。
- `BattleModifiers.forgeTiers` 和 `BattleState.forgeTiers` 保存战斗所需的品阶快照；战斗核心不直接读取 `PlayerSave`。
- 15 个熔炼对象为 3 把武器和 12 件装备；不实现第三品阶、第二地域材料、随机失败或装备销毁。
- 万象图鉴 60 件收藏和志怪录 22 个条目必须有稳定 `artKey`。未拥有/未发现条目不加载图片，已拥有/已发现条目使用固定尺寸懒加载 PNG。
- 正式素材继续通过开发期 GPT Image 单图生成和 FFmpeg 后处理完成；运行时不调用模型，不建立图集、Service Worker 或素材后台。资源预算为透明图单张 ≤160KB、横向图单张 ≤80KB、像素目录总量 ≤12MB。

## 17. M5.6.1 沉浸式战斗场景边界

- 斗法页采用“顶部资源 HUD、中央 Pixi 战场、底部手牌、右侧可折叠斗法录”的布局；灵力、剑意、计时和道具不覆盖战场画布。
- `src/ui/battle-visuals.ts` 是 Pixi 与 DOM 单位信息牌共用的纯布局和视觉 cue 来源；它只读取 `BattleState`/`BattleEvent`，不参与规则结算。
- 主将、妖灵和敌人立绘由 Pixi 绘制，名称、生元、攻势、护体、护盾、状态、灵契和下一次行动由 DOM 绘制；目标选择仍通过可聚焦 DOM 按钮完成。
- 缺少正式素材时使用墨影首字占位；Pixi 失败时保留背景、DOM 单位牌、手牌、结算和战报，不引入新的战斗规则或存档字段。
- 战斗视觉 cue 必须由事件确定性生成；页面隐藏或减弱动态时停止循环 ticker，不改变战斗状态和事件日志。
