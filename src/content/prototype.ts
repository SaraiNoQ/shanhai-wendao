import type { BattleContent, BuildId, BuildPreset, CardDefinition, CardId, SpiritDefinition, SpiritId, TechniqueId, WeaponId } from '../game/types'
...
const cards = {
  guiding_edge: card('guiding_edge', '引锋式', 1, '剑式', ['sword'], 'sword_strike', '造成 80% 攻势伤害，获得 2 点剑意。', { powerPercent: 80, swordIntent: 2 }),
  ...
  fire_talisman: card('fire_talisman', '贴火符', 1, '符箓', ['talisman'], 'apply_marks_and_burn', '施加 2 层符印和 1 层灼烧。', { marks: 2, burn: 1 }),
  ...
  call_true_name: card('call_true_name', '唤名', 1, '御灵', ['spirit'], 'gain_lowest_bond', '灵契最低的妖灵获得 1 点灵契。', {}, 'none'),
...
const spirits = {
  blade_tail_fox: spirit('blade_tail_fox', '刃尾狐', '三式追击', 96, 17, 10, 4_500, ['sword']),
  iron_beak_crane: spirit('iron_beak_crane', '铁喙鹤', '重击破甲', 108, 21, 14, 5_500, ['sword']),
  paper_bride: spirit('paper_bride', '纸嫁娘', '纸衣镇魂', 94, 15, 11, 4_800, ['talisman'], 'spirit_paper_bride'),
  lantern_ghost: spirit('lantern_ghost', '灯笼鬼', '鬼火灼魂', 88, 18, 8, 4_200, ['talisman']),
  mountain_child: spirit('mountain_child', '山童', '负石护主', 126, 14, 20, 5_200, ['spirit']),
  dream_tapir: spirit('dream_tapir', '食梦貘', '吞梦回灵', 102, 16, 13, 4_600, ['spirit']),
...
export const PROTOTYPE_CONTENT = {
  ...
  spirits,
  enemies: [{ id: 'clay_idol', name: '泥胎傀', title: '槐阴古道 · 负土成形', maxHp: 540, attack: 18, defense: 26, attackIntervalMs: 3_200, artKey: 'enemy_clay_idol' }],
  weapons: {
