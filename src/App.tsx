...
import './App.css'
// M5.6.1: HUD reuses the existing battle state and event stream; no new rules or save fields.
...
import { getBattleUnitLayout, type BattleUnitVisual } from './ui/battle-visuals'
...
function UnitCard({ unit, bond, enemy = false, selectable = false, compact = false, timeMs, onSelect }: { unit: UnitState; bond?: number; enemy?: boolean; selectable?: boolean; compact?: boolean; timeMs?: number; onSelect?: () => void }) {
  const art = unit.artKey ? artFiles[unit.artKey] : undefined
  return <article className={`unit-card ${enemy ? 'enemy-card' : ''} ${selectable ? 'is-selectable' : ''} ${compact ? 'unit-card-compact' : ''}`}>
    ...
    <div className="unit-copy"><p className="unit-title">{unit.title}</p><h3>{unit.name}</h3><div className="health-line" aria-label={`生元 ${unit.hp}/${unit.maxHp}`}><span style={{ width: `${Math.max(0, unit.hp / unit.maxHp * 100)}%` }} /></div>
      <div className="unit-stats"><span>生元 {unit.hp}/{unit.maxHp}</span><span>攻势 {unit.attack}</span><span>护体 {unit.defense}</span></div>{timeMs !== undefined && <span className="unit-next-action">下次出手 {Math.max(0, (unit.nextActionAtMs - timeMs) / 1_000).toFixed(1)} 秒</span>}
...
function BattleUnitLayer({ battle, targetingCard, onSelectSpirit }: { battle: BattleState<BattleCardReference>; targetingCard?: CardId; onSelectSpirit: (targetId: UnitId) => void }) {
  const layout = getBattleUnitLayout(battle)
  const allies = [battle.leader, ...battle.spirits]
  return <div className="battle-unit-layer" aria-label="斗法单位">
    {layout.map((visual: BattleUnitVisual) => {
      const unit = visual.side === 'ally' ? allies[visual.index] : battle.enemies[visual.index]
      if (!unit) return null
      const selectable = visual.side === 'ally' && visual.index > 0 && Boolean(targetingCard && unit.hp > 0)
      return <div key={visual.key} className={`battle-unit-anchor ${visual.side === 'enemy' ? `is-enemy ${battle.enemies.length > 1 ? 'is-group' : ''}` : visual.index === 0 ? 'is-leader' : 'is-spirit'} ${unit.hp > 0 ? '' : 'is-defeated'}`} style={{ left: `${visual.x * 100}%`, top: `${visual.y * 100}%` }}>
        <UnitCard unit={unit} enemy={visual.side === 'enemy'} compact bond={visual.side === 'ally' && visual.index > 0 ? battle.spiritBonds[visual.index - 1] : undefined} timeMs={battle.timeMs} selectable={selectable} onSelect={selectable ? () => onSelectSpirit(unit.id) : undefined} />
      </div>
    })}
  </div>
}

function BattleHud({ battle, buildName, weaponName, techniqueName, targetingCard, onUseTreasure, onUseConsumable, onOpenCharacter }: { battle: BattleState<BattleCardReference>; buildName: string; weaponName: string; techniqueName: string; targetingCard?: CardId; onUseTreasure: () => void; onUseConsumable: (id: string, slot: number) => void; onOpenCharacter: () => void }) {
  const livingEnemies = battle.enemies.filter((unit) => unit.hp > 0)
  return <section className="battle-hud" aria-label="斗法状态栏">
    <div className="hud-stat hud-energy"><small>灵力</small><strong>{battle.energy}<span> / {battle.maxEnergy}</span></strong><div className="hud-pips">{Array.from({ length: battle.maxEnergy }, (_, index) => <i key={index} className={index < battle.energy ? 'filled' : ''} />)}</div></div>
    <div className="hud-stat hud-sword"><small>剑意</small><strong>{battle.swordIntent}<span> / {battle.swordIntentCap}</span></strong></div>
    <div className={`hud-stat hud-time ${battle.timeMs >= 60_000 ? 'is-warning' : ''}`}><small>斗法计时</small><strong>{(battle.timeMs / 1_000).toFixed(1)} 秒</strong><span>{battle.timeMs >= 150_000 ? '输出不足警告 · 剩余 30 秒' : battle.timeMs >= 60_000 ? '久战未决' : '战斗进行中'}</span></div>
    <div className="hud-build"><small>当前构筑</small><strong>{buildName}</strong><span>{weaponName} · {techniqueName}</span></div>
    <div className="hud-target-status"><small>敌方状态</small>{livingEnemies.length ? livingEnemies.map((unit, index) => <span key={`${unit.id}-${index}`}>{unit.name} · 符印 {unit.talismanMarks} · 灼烧 {unit.burnStacks}</span>) : <span>战场已清空</span>}</div>
    <div className="hud-combos"><small>连携</small>{battle.activeCombos.length ? battle.activeCombos.map((id) => <span key={id}>{comboNames[id]}</span>) : <span>未激活</span>}</div>
    <div className="hud-actions"><div className="hud-items">{battle.treasureId && <button type="button" disabled={battle.status !== 'active' || battle.treasureCharge < battle.treasureMaxCharge} onClick={onUseTreasure}>法宝 {battle.treasureCharge}/{battle.treasureMaxCharge}</button>}{Object.entries(battle.consumableUses).map(([id, uses], index) => <button type="button" key={id} disabled={battle.status !== 'active' || uses <= 0} onClick={() => onUseConsumable(id, index)}>{COLLECTION_BY_ID[id]?.name ?? id} {uses}</button>)}</div><button type="button" className="hud-priority-button" onClick={onOpenCharacter}>调整出牌顺序</button></div>
    {targetingCard && <p className="hud-targeting-note" role="status">为「{names[targetingCard]}」选择存活妖灵 · 按 Esc 取消</p>}
  </section>
}

function BattleHand({ battle, battleContent, enemy, activeTargetCard, activeTargetInstance, onPlayCard }: { battle: BattleState<BattleCardReference>; battleContent: ReturnType<typeof battleContentFromSave>; enemy?: UnitState; activeTargetCard?: CardId; activeTargetInstance?: string; onPlayCard: (cardId: CardId, instanceId?: string) => void }) {
  return <div className="hand-zone"><div className="hand-heading"><span>{PROTOTYPE_CONTENT.builds[battle.buildId].name} · {battleContent.weapons[battleContent.builds[battle.buildId].weaponId].name}</span><span>手牌 {battle.hand.length}/4</span><span>牌库 {battle.deck.length}</span><span>弃牌 {battle.discard.length}</span></div><div className="hand-cards">{battle.hand.map((cardRef) => { const cardId = getCardId(cardRef); const instanceId = isBattleCardInstance(cardRef) ? cardRef.instanceId : undefined; const card = battleContent.cards[cardId]; const availability = getCardAvailability(battle, card, undefined, false, cardRef); const art = card.artKey ? artFiles[card.artKey] : undefined; const unavailable = availability.reason === 'battle_ended' ? '战斗已结束' : availability.reason === 'insufficient_energy' ? '灵力不足' : availability.reason === 'no_living_spirit' ? '无存活妖灵' : undefined; const hint = availability.reason === 'target_required' ? '点击后选择妖灵' : undefined; const previewDamage = card.powerPercent === undefined || card.effectId === 'spirit_basic_attacks' || card.effectId === 'spirit_tide' ? undefined : getDamageBreakdown({ attack: battle.leader.attack, powerPercent: card.powerPercent, defense: enemy?.defense ?? 0, armorBreak: enemy?.armorBreak, hits: card.hits }); return <button type="button" key={instanceId ?? cardId} className={`hand-card archetype-${card.tags[0]} ${activeTargetCard === cardId && activeTargetInstance === instanceId ? 'is-targeting' : ''} ${unavailable ? 'is-unavailable' : ''}`} disabled={Boolean(unavailable)} onClick={() => onPlayCard(cardId, instanceId)}><span className="card-cost">{availability.cost}</span><span className="card-kind">{card.kind}</span><strong>{card.name}</strong><span className="card-illustration" aria-hidden="true">{art ? <img src={`${ASSET_ROOT}${art}`} alt="" /> : <i>{card.name.at(0)}</i>}</span><span className="card-text">{card.description}</span>{previewDamage && <span className="card-preview-value">基础伤害 {previewDamage.damagePerHit} × {previewDamage.hits} = {previewDamage.totalDamage}</span>}{(unavailable || hint) && <span className="card-unavailable">{unavailable ?? hint}</span>}</button> })}</div></div>
}

function BattleEventList({ events, limit }: { events: readonly BattleEvent[]; limit?: number }) {
  const visible = events.slice().reverse().slice(0, limit)
  return <ol className="event-list">{visible.map((event, index) => { const [time, copy] = formatEvent(event); return <li key={`${event.atMs}-${event.type}-${index}`}><time>{time}s</time><span>{copy}</span></li> })}</ol>
}

function BattleLog({ events }: { events: readonly BattleEvent[] }) {
  const [expanded, setExpanded] = useState(false)
  return <aside className="intel-panel"><section className="event-panel" aria-live="polite"><div className="panel-heading compact"><span>肆</span><div><small>BATTLE RECORD</small><h2>斗法录</h2></div></div><button type="button" className="log-toggle" aria-expanded={expanded} aria-controls="battle-log-list" onClick={() => setExpanded((value) => !value)}>{expanded ? '收起最近战报' : '展开完整日志'}（{events.length} 条）</button><div id="battle-log-list"><BattleEventList events={events} limit={expanded ? undefined : 3} /></div></section></aside>
}
...
function App() {
  ...
  const battleView = page === 'battle' && <>
    <BattleHud battle={battle as BattleState<BattleCardReference>} buildName={PROTOTYPE_CONTENT.builds[battle.buildId].name} weaponName={battleContent.weapons[build.weaponId].name} techniqueName={battleContent.techniques[build.techniqueId].name} targetingCard={targetingCard} onUseTreasure={() => dispatch({ type: 'use_treasure', treasureId: battle.treasureId! })} onUseConsumable={(id, slot) => dispatch({ type: 'use_consumable', consumableId: id, slot })} onOpenCharacter={openCharacter} />
    <main className="battle-layout">
      <section className={`battle-table ${campaignSession && campaignSession.stageNumber > 20 ? 'region-roots' : campaignSession && campaignSession.stageNumber > 10 ? 'region-waystation' : ''}`}>
        <div className="battle-stage">
          <Suspense fallback={<div className="battle-scene-loading" role="status">正在铺开斗法场景……</div>}><BattleScene battle={battle as BattleState<BattleCardReference>} events={activeEvents} backgroundUrl={battleBackgroundUrl} reducedMotion={reducedMotion} /></Suspense>
          <div className="scene-vignette" aria-hidden="true" />
          <BattleUnitLayer battle={battle as BattleState<BattleCardReference>} targetingCard={targetingCard} onSelectSpirit={chooseSpiritTarget} />
        </div>
        <BattleHand battle={battle as BattleState<BattleCardReference>} battleContent={battleContent} enemy={enemy} activeTargetCard={targetingCard} activeTargetInstance={targetingInstanceId} onPlayCard={clickCard} />
        {battle.status !== 'active' && <div className={`battle-result ${battle.status}`} role="status"><span>{battle.status === 'victory' ? '破' : '败'}</span><h2>{battle.status === 'victory' ? '试法告捷' : '心脉受创'}</h2><p>{trialBattle ? '劫境战斗已结束，返回地图结算本格。' : battle.status === 'victory' ? `「${PROTOTYPE_CONTENT.builds[battle.buildId].name}」已证可行。` : '此关未能通过，系统已记录失败原因。'}</p><div className="battle-result-actions"><button type="button" className="result-primary" onClick={finishBattle}>{trialBattle ? '返回劫境并结算' : '返回游历并结算'}</button><button type="button" onClick={() => dispatch({ type: 'restart' })}>重试本关</button></div></div>}
      </section>
      <BattleLog events={activeEvents} />
    </main>
  </>
